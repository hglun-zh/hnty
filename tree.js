...
.from('.treeStar', {
 ...
 .to('.sparkle', {
	duration: 1,
    opacity:1,
    ease:"rough({strength: 10, points: 100, template: linear, taper: both, randomize: true, clamp: false})"
  },'-=0')
  .to('.treeStarOutline', {
	duration: 1,
    opacity:1,
    ease:"rough({strength: 10, points: 100, template: linear, taper: none, randomize: true, clamp: false})"
  },'+=0')
...
function textAnim() {
    ...
    // Hiệu ứng nhấp nháy
    textTl.fromTo('.merryText', 0.5, { opacity: 0 }, { opacity: 1, ease: Power1.easeInOut })
           .fromTo('.merryText', 0.5, { opacity: 1 }, { opacity: 0, ease: Power1.easeInOut });
  
    mainTl.add(textTl, 0); // Điều chỉnh vị trí trong dòng thời gian nếu cần thiết
  }
  
  // Gọi hàm animation cho văn bản
  textAnim();
